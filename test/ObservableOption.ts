...
describe('ObservableOption', () => {
  ...
  it('fold left', async () => {
    ...
    const e = await pipe(_.none, _.fold(f, g))
      .pipe(bufferTime(10))
      .toPromise()
  ...
  it('fold some', async () => {
    const f = (): Observable<number> => rxOf(2)
    const g = (n: number): Observable<number> => rxOf(n * 3)
    const e = await pipe(_.some(3), _.fold(f, g))
      .pipe(bufferTime(10))
      .toPromise()
  ...
  it('getOrElse (left)', async () => {
    ...
    const e = await pipe(_.none, _.getOrElse(onLeft))
      .pipe(bufferTime(10))
      .toPromise()
  ...
  it('getOrElse (some)', async () => {
    ...
    const e = await pipe(_.some(1), _.getOrElse(onLeft))
      .pipe(bufferTime(10))
      .toPromise()
  ...
  describe('Monad', () => {
    ...
    it('map', async () => {
      ...
      const x = await pipe(_.some(1), _.map(double))
        .pipe(bufferTime(10))
        .toPromise()
    ...
    it('chain', async () => {
      ...
      const e1 = await pipe(_.some('four'), _.chain(f))
        .pipe(bufferTime(10))
        .toPromise()
      assert.deepStrictEqual(e1, [O.some(4)])
      const e2 = await pipe(_.some('a'), _.chain(f))
        .pipe(bufferTime(10))
        .toPromise()
      assert.deepStrictEqual(e2, [O.none])
      const e3 = await pipe(_.none, _.chain(f))
        .pipe(bufferTime(10))
        .toPromise()
    ...
    it('left identity', async () => {
      const f = (a: string): _.ObservableOption<number> => (a.length > 2 ? _.some(a.length) : _.none)
      const a = 'text'
      const e1 = await pipe(_.of<string>(a), _.chain(f))
        .pipe(bufferTime(10))
        .toPromise()
      const e2 = await f(a)
        .pipe(bufferTime(10))
  ...
  it('chainFirst', async () => {
    const f = (a: string): _.ObservableOption<number> => (a.length > 2 ? _.some(a.length) : _.none)
    const e1 = await pipe(_.some('aaaa'), _.chainFirst(f))
      .pipe(bufferTime(10))
      .toPromise()
  ...
  it('fromOption', async () => {
    assert.deepStrictEqual(
      await _.fromOption(O.some(1))
        .pipe(bufferTime(10))
        .toPromise(),
      [O.some(1)]
    )
    assert.deepStrictEqual(
      await _.fromOption(O.none)
        .pipe(bufferTime(10))
        .toPromise(),
      [O.none]
    )
